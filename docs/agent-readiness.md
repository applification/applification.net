...
The floating control animates route changes with a 450ms rectangular View Transition: bottom-to-top into Agent and top-to-bottom into Human. It waits for the destination route to commit so the snapshot includes its palette. A 1.5-second capture limit lets slow navigation continue without freezing the page. It shares the existing colour-switcher transition styling and guard, preserves the saved Human preference, and falls back to native link navigation for reduced motion, unsupported browsers and JavaScript-free browsing. Modified clicks retain native new-tab behaviour.

Public content pages expose a small Human / Agent control floating at the bottom centre. The separate bot link stays in the header on every route, on mobile and when scrolled. `/agent` shows the homepage as Markdown inside the site shell; `/agent/<page-path>` shows the selected page. This reader has a dedicated graphite dark theme, compact mono heading and a document toolbar. The theme applies to the full shell without changing the saved Human theme, which returns when the reader is left. `/markdown` and `/markdown/<page-path>` return the same complete document as `text/markdown`, with CORS, a canonical Link header and a one-hour cache. The page advertises a Markdown alternate and `llms.txt` documents the URL pattern. The Agent HTML view has a canonical URL pointing back to the human page and is marked noindex to avoid duplicate search entries.
...
The Agents guide offers logo buttons for ChatGPT, Claude, Perplexity, Grok and Gemini. The first four encode the visible starter prompt in their web URLs (`chatgpt.com/?q=...`, `claude.ai/new?q=...`, `perplexity.ai/search/new?q=...`, `grok.com/?q=...`). Gemini's web URL opened with an empty composer in testing, so its action copies the prompt and opens `gemini.google.com/app`, with instructions to paste and a manual-copy fallback if clipboard access fails. The Markdown view includes the same destinations and prompt. Links retain native new-tab navigation, accessible names, tooltips and 44px targets. SVG marks are bundled from LobeHub with its MIT notice in `/licenses/lobehub-icons.txt`.

The assistant controls sign-in and whether it prefills or submits the prompt, so the page does not promise a review step or automatic web access. Visitors can copy the prompt if it does not survive the handoff, or paste Markdown if web fetching fails. No native assistant connector is claimed or installed. Browser-safe skill descriptions are separated from the server-side discovery digest so Storybook can render the guide.
...
The Agents guide includes an editable starter prompt with reset and copy actions. Named assistant buttons use native GET forms so edits reach ChatGPT, Claude, Perplexity and Grok even without JavaScript. Gemini opens a plain chat URL and copies the current text for pasting; the page provides manual-copy guidance when JavaScript or clipboard access is unavailable. Public Markdown continues to publish the default starter prompt and deep links, never a visitor’s edits.
