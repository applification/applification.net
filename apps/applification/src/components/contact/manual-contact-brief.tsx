...
export function ManualContactBrief({ draft, originalMessage, onRoute, onField, onReview }: {
  ...
  return (
    <form className="flex flex-col bg-[var(--app-section)]" aria-label="Complete enquiry manually" onSubmit={(event) => { event.preventDefault(); if (valid) onReview(); }}>
      ...
      <div className="mx-auto grid max-w-[760px] gap-3">
        ...
        <div className="grid grid-cols-1 items-start gap-x-5 gap-y-3 sm:grid-cols-2" data-manual-fields>
          ...
          {([...fields[route], ["replyName", "Your name"], ["replyEmail", "Reply email"], ["summary", "Overview / additional details"]] as Array<readonly [TextField, string]>).map(([field, label]) => {
            ...
            return <Field className={cn("min-w-0 gap-0.5", long && "sm:col-span-2")} key={field}>
              <FieldLabel htmlFor={id} className="text-[13px] font-semibold">{label}{optional.has(field) ? " (optional)" : ""}</FieldLabel>
              {long ? <Textarea id={id} aria-describedby={`${id}-count`} required={!optional.has(field)} maxLength={contactTextLimits[field]} rows={optional.has(field) ? 2 : 3} className={cn("resize-y bg-[var(--contact-input)] text-base", optional.has(field) ? "min-h-16" : "min-h-20")} value={draft[field] ?? ""} onChange={(event) => onField(field, event.target.value)} />
      ...
      <div className="border-t border-[var(--app-border)] p-4 sm:px-6" data-manual-actions>
