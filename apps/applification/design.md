...
The `/agents` guide leads with the human benefit: exploring Dave's work with a preferred AI assistant, with web access enabled and a visible starter prompt. Offer a row of 44px outlined logo buttons for ChatGPT, Claude, Perplexity, Grok and Gemini, followed by an outlined copy action. Use authentic monochrome marks, accessible action names and tooltips announcing the new tab. These logo-only controls omit the trailing arrow; ordinary external text links retain it. The first four links carry the visible prompt. Gemini copies the prompt and opens its chat, with feedback to paste it there because its web URL does not prefill reliably. Keep all five logos together at 320px; wrap the copy action below on small screens. Explain sign-in and prompt handoff without promising automatic web access or a native chat connector. Keep the API docs, sandbox, skills and expandable reference below this introduction. Pricing and commercial terms remain in the catalog JSON and tool responses, not a visible commercial section added for scanner coverage.
