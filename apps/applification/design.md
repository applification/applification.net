...
The `/agents` guide leads with the human benefit: exploring Dave's work with ChatGPT or Claude, with web access enabled and a visible starter prompt. Offer direct web links to both assistants using that same prompt, followed by an outlined copy action as a fallback. Keep the links on the shared button treatment with external-link cues; wrap the actions on small screens. Explain sign-in and prompt handoff without promising automatic web access or a native chat connector. Keep the API docs, sandbox, skills and expandable reference below this introduction. Pricing and commercial terms remain in the catalog JSON and tool responses, not a visible commercial section added for scanner coverage.
