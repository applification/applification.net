...
The bot icon is a permanent link to the Agents guide in the header, including mobile and the compact scrolled state. Keep it independent of the view switch. Public content pages use a small Human / Agent control floating at the bottom centre, inspired by Parallel. Use 12px labels, a muted 32px visual surface and 44px hit targets, with a dot and text weight showing the selected view. Respect the bottom safe area, keep it below menus and dialogs, and leave clearance after the footer links. Never put this control in the header navigation. It changes the current page's view and keeps section navigation in Agent mode until the visitor switches back. Agent view shows Markdown with copy and direct-link actions, plus a link to the `/agents` guide. Keep contact workflows and private previews out of this view. At narrow desktop widths, show the logo mark and tighten navigation spacing before shrinking controls. Product pages keep their theme in Human view.

Agent view has a dedicated, always-dark graphite palette across the reader, header, footer and floating control. It stays distinct from the ordinary slate-blue dark theme. Use IBM Plex Mono for compact reader headings, Geist for supporting copy, and Geist Mono for the complete Markdown document. Frame the source with a quiet border and a toolbar showing its Markdown path, format and copy/open actions. Reserve the existing pale cyan for orientation, links and focus. Hide the colour-theme control in Agent view, preserve the visitor’s Human theme preference, and restore it when they leave. Select this palette from the server-rendered reader so direct loads and JavaScript-free navigation receive the same treatment.

The floating view control uses a 450ms rectangular reveal: graphite rises from the bottom for Agent, and the Human page returns from the top. Capture the destination after its route and palette render, using the shared View Transition styling. Release the capture if navigation takes longer than 1.5 seconds. Reduced motion and unsupported browsers use ordinary navigation; native links still support modified clicks and JavaScript-free browsing. Keep the colour-theme control’s circular reveal independent.

The `/agents` guide leads with the human benefit: exploring Dave's work with a preferred AI assistant, with web access enabled and an editable starter prompt. Use the shared Textarea with a distinct card fill, a “Your prompt” label, a short hint and a “Reset prompt” action. Put named ChatGPT, Claude, Perplexity, Grok and Gemini buttons below it, using 18px monochrome logos, visible names and a small trailing external arrow. Use content-width buttons in a wrapping row, with a quiet card fill, fine border and 36px visible surface inside a 44px hit target. Avoid full-width grid cells or primary action fills for these peer choices. Put a small “Copy prompt” icon button beside “Reset prompt” above the field, with a 44px hit target, accessible name and title. Show copy feedback with the shared shadcn Sonner toast, never an inline message that shifts the form. Success dismisses after four seconds; failure guidance after six. Use the shared palette and place the toast above the floating view control, including mobile safe areas. Repeated copies update one toast; edits, reset and leaving the composer dismiss it. Native GET forms send the current edited text to the first four assistants, including without JavaScript. Gemini copies the current prompt and opens a plain chat URL, with feedback to paste it there because its web URL does not prefill reliably. Copy failures select the editable text and explain the next step. Clear stale copy feedback when the prompt changes, disable empty-prompt actions and let Reset restore the starter. Keep accessible new-tab names and visible focus. Explain sign-in and prompt handoff without promising automatic web access or a native chat connector. Keep the API docs, sandbox, skills and expandable reference below this introduction. Pricing and commercial terms remain in the catalog JSON and tool responses, not a visible commercial section added for scanner coverage.
...
Do not set whole paragraphs or tables in mono, except the raw Markdown document in Agent view. Do not use a tracked uppercase label as decoration. A label must identify a content type, state, source, or step that helps the reader navigate.
...
In the compact desktop header, hide navigation links while scrolling down. Keep the logo, bot link and theme switcher visible. Restore navigation on upward scrolling, near the top, or when keyboard focus enters the header.
