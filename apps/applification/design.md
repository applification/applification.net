...
The `/agents` guide leads with the human benefit: exploring Dave's work with ChatGPT or Claude, with web access enabled and a copyable starter prompt. Keep the API docs, sandbox, skills and expandable reference below this introduction. Describe browsing accurately; the site does not establish a native chat connector. Pricing and commercial terms remain in the catalog JSON and tool responses, not a visible commercial section added for scanner coverage.
